...
module.exports.load = function(config, deploy) {
  ...
  // lambda with same file but different method produces separate lambda

  var method = config.Handler.split('.').pop();
  var key = config.file + '.' + method;

  return cache[key] || (cache[key] = new Lambda(config, deploy));
...
function Lambda(config, deploy) {
  ...
  this.stage += '' + deploy.version;
  this.file = config.file;
  this.config = config;

  Object.defineProperty(this, 'method', {
    value: config.Handler.split('.').pop()
  });

  Object.defineProperty(this, 'storeFile', {
    value: this.file + '.' + this.method
  });
  ...
  this.name = this.stage + '--' + deploy.app + '--' + config.file + '--' + this.method;
  this.name = this.name
  .replace(/\./, '_')
  .replace(/\//g, '--')
  .replace(/\\/g, '--');
  ...
  Object.defineProperty(this, 'uri', {
    value: util.format(
      'arn:aws:apigateway:%s:lambda:path/2015-03-31/functions/arn:aws:lambda:%s:%s:function:%s/invocations',
                    deploy.region,                               deploy.region, deploy.account, this.name),
    enumerable: true,
  })
 ...
 * ### .toAddPermission()
 *
 * Returns this lambda as the AWS definition for LambdaApi.addPermission()
 *
 * @api public
 * @param ()
 * @param {AwsRestApi} awsRestApi
 * @param {AwsRestApiResource} awsApiMethod
 * @param {AwsRestApiMethod} awsApiMethod
 *
 */

Lambda.prototype.toAddPermission = function(awsRestApi, awsResource, awsApiMethod) {

  // console.log('RESTAPI', awsRestApi);
  // console.log('RESOURCE', awsResource);
  // arn:aws:apigateway:us-east-1::my-api-id:/test/petstorewalkthrough/pets

  var arn = 'arn:aws:apigateway:' + this.deploy.region + '::' + awsRestApi.id + ':' + awsResource.path;

  // All details into Sid,
  // used to determine if the permission has already been assigned (grep _permission)
  // used to delete no-longer necessary permissions on change

  var statementId = md5('arn_aws_apigateway_' + this.deploy.region + '__' + 
      awsRestApi.id + '_' + awsApiMethod.httpMethod + '-' + awsResource.path.replace(/\//g, '-') + '_' +
      this.name);
  ...
  return {
    ...
    // Can only get permission to work by skipping SourceArn,
    // this means that any AWS resource can invoke the lambda
    // (which is ok becuase auth is internal to the lambda)
    // 
    // - But spending some time trying to get tighter security to work 
    //   is a good idea. When time permits.
    //   
    //
    // SourceArn: 'arn:aws:apigateway:' + this.deploy.region + '::' + awsRestApi.id + ':' + awsResource.path,
...
Lambda.prototype.zip = function() {
  ...
  if (this.base64) {
    debug('already ziped for lambda: ', this.storeFile);
  }

  debug('loading zip for lambda: ', this.storeFile);

  var _this = this;

  var zip = new AdmZip();
  var zipFileName = process.cwd() + path.sep + 'deployed' + path.sep + this.stage + path.sep + _this.storeFile + '.zip'
  ...
  if (shasum.unchanged) {
    ...
    debug('no changes to zip for lambda: \'%s\'', this.storeFile);
