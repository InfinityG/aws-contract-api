 ...
 * @param {String} deployStage
 *
 */

module.exports.load = function(jsFile, version, deployStage) {

  return cache[jsFile] || (cache[jsFile] = new Lambda(jsFile, version, deployStage));
...
function Lambda(jsFile, version, deployStage) {

  this.stage = deployStage || 'development';

  this.stage += '_' + version;

  var app = process.cwd().split(path.sep).pop();
  ...
  this.name = deployStage + '--' + app + '--' + jsFile
  .replace(/\./, '_')
  .replace(/\//g, '--')
  .replace(/\\/g, '--')

  debug('create lambda: \'%s\', name: \'%s\', for stage: \'%s\'', jsFile, this.name, this.stage);
...
Lambda.prototype.zip = function() {
  ...
  var zip = new AdmZip();
  var zipFileName = process.cwd() + path.sep + 'zips' + path.sep + this.stage + path.sep + _this.file + '.zip'
