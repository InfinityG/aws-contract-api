...
var Promise = require('bluebird') // available from parent's modules
  ...
  , updateApiMethodResponses = require('debug')('Deployer:updateApiMethodResponses')
    ...
    return Promise.map(awsModels, function(awsModel) {
      ...
      if (!model) {
        ...
        .then(function(result) {
          ...
          updateApiModels('name: \'%s\' model DELETED', awsModel.name);
...
module.exports.updateApiResourceMethod = function(aws, apiTree, awsRestApi, awsResource, apiMethod) {
  ...
  .then(function(result) {
    ...
    console.log('RESULT OF API UPDATE METHOD RESPONSES', result);
...
module.exports.updateApiMethodResponses = function(aws, apiTree, apiMethod, awsResource, awsApiMethod) {
  ...
  return Promise.map(awsMethodResponses, function(awsMethodResponse) {
    ...
    if (patch.length == 0) {
      ...
      return updateApiMethodResponses(
  ...
  .then(function() {
    ...
    return Promise.map(addList, function(statusCode) {
      ...
      var apiResponseMethod = responses[statusCode];

      console.log('AAAADDD', apiResponseMethod);
...
module.exports.createOrUpdateLambda = function(aws, apiTree, apiMethod) {
  ...
  createOrUpdateLambda('lambda: \'%s\'', lambda.storeFile);
  ...
  return new Promise(function(resolve, reject) {
    ...
    .then(function(result) {
      ...
      if (patch.length > 0) {
        unchanged = false;
        createOrUpdateLambda('lambda: \'%s\' UPDATED config \'%j\'', lambda.storeFile, patch);
    ...
    .then(function(result) {
      ...
      if (awsLambdaVersion != lambda.sha256) {
        unchanged = false;
        createOrUpdateLambda('lambda: \'%s\' UPDATED code', lambda.storeFile);
    ...
    .then(function(result) {
      if (unchanged) {
        createOrUpdateLambda('lambda: \'%s\' unchanged', lambda.storeFile);
    ...
    .catch(function(error) {
      ...
      .then(function(result) {
        createOrUpdateLambda('lambda: \'%s\' CREATED', lambda.storeFile);
        return aws.lambda.getFunction(lambda);
      })
