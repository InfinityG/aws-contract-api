...
module.exports.updateApiMethodResponses = function(aws, apiTree, apiMethod, awsResource, awsApiMethod) {
  ...
  return Promise.map(awsMethodResponses, function(awsMethodResponse) {
    ...
    aws.apigw.removePatchNulls(['/responseModels', '/responseParameters'], patch);
...
module.exports.updateApiMethodIntegration = function(aws, apiTree, apiMethod, awsRestApi, awsResource, awsApiMethod) {
  ...
  .then(function() {
    ...
    return module.exports.updateApiMethodIntegrationResponses(aws, apiTree, apiMethod, awsResource, awsApiMethodIntegration);
...
/**
 * ### .updateApiMethodIntegrationResponses()
 *
 * @api public
 * @param {AwsApiList} aws
 * @param {ApiTree} apiTree
 * @param {ApiMethod} apiMethod
 * @param {AwsRestApiResource} awsResource
 * @param {AwsApiMethodIntegration} awsApiMethodIntegration
 * @returns {Promise}
 *
 *
 */

module.exports.updateApiMethodIntegrationResponses = function(aws, apiTree, apiMethod, awsResource, awsApiMethodIntegration) {

  var integration = apiMethod.definition.integration;
  var responses = integration.responses;
  var awsIntegrationResponses;
  var done = [];

  try {
    awsIntegrationResponses = awsApiMethodIntegration._embedded['integration:responses'] || [];
    if (!Array.isArray(awsIntegrationResponses)) awsIntegrationResponses = [awsIntegrationResponses];
  } catch (e) {
    awsIntegrationResponses = [];
  }

  return Promise.map(awsIntegrationResponses, function(awsIntegrationResponse) {

    var statusCode = awsIntegrationResponse.statusCode;
    var apiIntegrationResponse = responses[statusCode];

    if (!apiIntegrationResponse) {

      return aws.apigw.deleteRestApiMethodIntegrationResponse(awsIntegrationResponse)

      .then(function() {

        debug(
          'method: \'%s\', path: \'%s\', response: \'%s\' integration DELETED',
          apiMethod.method,
          awsResource.path,
          statusCode
        );
    ...
    var patch = diff(
      awsIntegrationResponse.excludeKeys([
        'statusCode',
        '_links',
        '_embedded'
      ]),
      apiIntegrationResponse.excludeKeys([
        '_aws',
        '_log',
      ])
    );

    aws.apigw.removePatchNulls(['/responseTemplates', '/responseParameters'], patch);

    if (patch.length == 0) {

      done.push(statusCode);
      return debug(
        'method: \'%s\', path: \'%s\', response: \'%s\' integration unchanged',
        apiMethod.method,
        awsResource.path,
        statusCode
      );
    ...
    return aws.apigw.patchRestApiMethodIntegrationResponse(awsIntegrationResponse, patch)

    .then(function() {

      done.push(statusCode);
      apiTree.updated(apiIntegrationResponse);
      debug(
        'method: \'%s\', path: \'%s\', response: \'%s\' integration UPDATED',
        apiMethod.method,
        awsResource.path,
        statusCode
      );
  ...
  .then(function() {
    ...
    return Promise.map(addList, function(statusCode) {
      ...
      apiIntegrationResponse.statusCode = statusCode;

      return aws.apigw.createRestApiMethodIntegrationResponse(apiIntegrationResponse, awsApiMethodIntegration)

      .then(function(awsIntegrationResponse) {

        apiTree.updated(apiIntegrationResponse);
        return debug(
          'method: \'%s\', path: \'%s\', response: \'%s\' integration CREATED',
          apiMethod.method,
          awsResource.path,
          statusCode
        );
