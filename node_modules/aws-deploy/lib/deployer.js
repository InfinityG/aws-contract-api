...
module.exports.updateApiResourceMethod = function(apigw, apiTree, awsResource, apiMethod) {
  ...
  if (!methodLinks) return module.exports.createApiResourceMethod(apigw, apiTree, awsResource, apiMethod);

  if (!Array.isArray(methodLinks)) {
    if (methodLinks.name != apiMethod.method) {
      return module.exports.createApiResourceMethod(apigw, apiTree, awsResource, apiMethod);
    }
    methodLink = methodLinks;
  }
  else if (
    methodLinks.filter(function(methodLink) {
      if (methodLink.name == apiMethod.method) {
        methodLink = methodLinks;
        return true;
      }
      return false;
...
module.exports.createApiResourceMethod = function(apigw, apiTree, awsResource, apiMethod) {
  ...
  method.authorizationType = method.authorizationType || 'NONE';
  method.apiKeyRequired = typeof method.apiKeyRequired == 'boolean' ? method.apiKeyRequired : false;
  method.requestParameters = method.requestParameters || {};
  method.requestModels = method.requestModels || {};

  // apigw.createApiResourceMethod()
